...
const V2_DRAFT_KEY = "v2conversational:draft:v1";
/**
 * When this flag is set to "1", the next ModerationProvider mount will prefer the V2 draft
 * (even if there is an existing legacy draft in localStorage).
 */
const V2_APPLY_ONCE_KEY = "v2conversational:apply_draft_once";

// Legacy keys we may want to fall back to if the provider storageKey doesn't exist.
const LEGACY_DRAFT_KEYS = ["campaign:moderation:draft", STORAGE_KEY_FALLBACK];

type V2CampaignDraft = {
  version?: number;
  ...
  // Step 2 / 3 fields generated by the conversational builder
...
function readV2Draft(): V2CampaignDraft | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(V2_DRAFT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object") return null;
    return parsed as V2CampaignDraft;
  } catch {
    return null;
  }
}

function reviveFromV2Draft(draft: V2CampaignDraft): ModerationCampaign {
  ...
  const goal = str(draft?.goal);
  const summary = str(draft?.summary);
  const leadDefinition = str(draft?.leadDefinition);

  const codeRaw = str(draft?.country?.code);
  const countryCode = codeRaw ? codeRaw.toUpperCase() : "";
  const country = str(draft?.country?.name);

  const channels: Channel[] = Array.isArray(draft?.channels)
    ? draft.channels.filter((c: any): c is Channel => CHANNELS.includes(c))
    : [];
  const webchatDomain = str(draft?.webchatDomain);

  const assistantDraft = draft?.assistant || {};
  const assistant: AssistantConfig = {
    ...base.assistant,
    name: str(assistantDraft.name),
    greeting: str(assistantDraft.greeting),
    conversationLogic: str(assistantDraft.conversationLogic),
    voiceFile: null,
    voiceUrl: null,
  };

  const knowHow: QA[] = Array.isArray(draft?.knowHow)
    ? draft.knowHow
        .map((item: any) => {
          const question = str(item?.question);
          const answer = str(item?.answer);
          if (!question || !answer) return null;
          const id = str(item?.id) || genId("qa");
          return { id, question, answer } as QA;
        })
        .filter(Boolean) as QA[]
    : [];

  const escalationItems = arr(draft?.escalationItems);
  const escalationPhone = str(draft?.escalationPhone);
  ...
  return {
    ...
    audience: {
      ...
      geo: {
        ...
        countryCode,
        country,
        countryIds: countryCode ? [countryCode] : [],
      },
    },
  };
}
...
export const ModerationProvider: React.FC<{
  ...
  const [data, setData] = useState<ModerationCampaign>(() => {
    ...
    // If V2 set the apply-once flag, prefer the conversational draft even if a legacy draft exists.
    const applyOnce = window.localStorage.getItem(V2_APPLY_ONCE_KEY) === "1";
    const v2 = readV2Draft();

    if (applyOnce) {
      try {
        window.localStorage.removeItem(V2_APPLY_ONCE_KEY);
      } catch {
        // ignore
      }
      if (v2) return reviveFromV2Draft(v2);
    }

    // Primary: provider storageKey
    const rawPrimary = window.localStorage.getItem(storageKey);
    if (rawPrimary) return reviveFromPersist(rawPrimary);

    // Fallback: known legacy keys
    for (const k of LEGACY_DRAFT_KEYS) {
      if (k === storageKey) continue;
      const raw = window.localStorage.getItem(k);
      if (raw) return reviveFromPersist(raw);
    }

    // Fallback: V2 canonical draft (if present)
    if (v2) return reviveFromV2Draft(v2);
