...
const Badge: React.FC<{
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[12px] ${cls}`}>
            {icon}
            {children}
        </span>
    );
...
/** Texto “cortado” prolijo + tooltip con el valor completo */
const TruncText: React.FC<{
    ...
    return (
        <span
            title={value}
            className={`block ${lineClamp} break-words ${className ?? ""}`}
        >
            {value}
        </span>
    );
...
export default function StepReview() {
    ...
    const productOrService = data?.summary || "";
                ...
                <div className="relative flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div className="min-w-0">
                        <div className="flex items-center gap-2 text-[12px] text-neutral-600 dark:text-neutral-400">
                            ...
                            <span className="uppercase tracking-wide">Resumen de campaña</span>
                        </div>
            ...
            >
                ...
                <motion.div variants={{ hidden: { opacity: 0, y: 8 }, show: { opacity: 1, y: 0 } }}>
                    <GlassPanel
                        ...
                        subtitle="Campos clave de configuración"
                    >
                        <div className="divide-y divide-neutral-200/60 dark:divide-neutral-800/60">
                            <StatRow label="Nombre de la Campaña" value={<TruncText text={data?.name} lines={1} />} />
                            <StatRow label="Producto o Servicio" value={<TruncText text={data?.summary} lines={2} />} />
                            <StatRow label="Objetivo" value={<TruncText text={data?.goal} lines={2} />} />
                            <StatRow label="Lead" value={<TruncText text={data?.leadDefinition} lines={2} />} />
                        </div>
                    </GlassPanel>
                </motion.div>
                <div className="grid grid-cols-2 gap-2">
                    <motion.div variants={{ hidden: { opacity: 0, y: 8 }, show: { opacity: 1, y: 0 } }}>
                        <GlassPanel
                            ...
                            title="Canales"
                            subtitle="Dónde el asistente interactuará"
                        >
                            {channels.length ? (
                    ...
                    >
                        <GlassPanel
                            ...
                            title="Lugar donde correrá"
                            subtitle="Segmentación geográfica configurada"
                        >
                            <div className="divide-y divide-neutral-200/60 dark:divide-neutral-800/60">
                                <StatRow label="Ubicación" value={<TruncText text={formatLocation(geo)} lines={2} />} />
                            </div>
                        </GlassPanel>
                    </motion.div>
                </div>
